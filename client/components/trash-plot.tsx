...
interface TrashPlotProps {
  trashBin: MonsterItem;
  isSelected?: boolean;
  onPress: (trashBin: MonsterItem) => void;
}
...
export function TrashPlot({ trashBin, isSelected, onPress }: TrashPlotProps) {
  return (
    <Marker
      key={trashBin.id}
      coordinate={{
    ...
    >
      <Pressable style={styles.trashMarkerContainer}>
        {({ pressed }) => (
          <View
            style={[styles.trashMarker, pressed && styles.trashMarkerPressed]}
          >
